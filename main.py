...
def fetch_data():
    try:
        ...
        mycursor.execute(
            "SELECT o.IdOcena, s.imie, s.Nazwisko, p.Nazwa, o.[Ocena]  FROM Student AS s JOIN Ocena AS o ON o.IdStudent=s.IdStudent JOIN Przedmiot AS p ON p.IdPrzedmiot = o.IdPrzedmiot")
    ...
    if selected_item:
        ...
        def update_data():
            try:
                ...
                mycursor.execute("UPDATE Ocena SET Ocena = (?) WHERE IdOcena = (?)",
                                 (ocena_entry.get(), id_entry.get()))
                mydb.commit()
            except sqlite3.Error as e:
                print(f"Error: {e}")
            finally:
